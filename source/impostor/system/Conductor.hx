package impostor.system;

import flixel.util.FlxSignal.FlxTypedSignal;

/**
 * The heart of all musical logic.
 * Manages timing, beats, measures, and BPM changes for rhythm-based gameplay.
 */
final class Conductor
{
	/**
	 * Gets triggered when a new measure is reached.
	 */
	public static var onMeasureHit:FlxTypedSignal<Int->Void> = new FlxTypedSignal<Int->Void>();

	/**
	 * Gets triggered when a new beat is reached.
	 */
	public static var onBeatHit:FlxTypedSignal<Int->Void> = new FlxTypedSignal<Int->Void>();

	/**
	 * Gets triggered when a new step is reached.
	 */
	public static var onStepHit:FlxTypedSignal<Int->Void> = new FlxTypedSignal<Int->Void>();

	/**
	 * Current position in the song in milliseconds.
	 */
	public static var songPosition(default, null):Float = 0.0;

	/**
	 * Current Beats Per Minute (can change during song).
	 */
	public static var curBPM(get, never):Float;

	/**
	 * Number of beats in a measure.
	 */
	public static var beatsPerMeasure(get, never):Int;

	/**
	 * Number of steps in a beat.
	 */
	public static var stepsPerBeat(get, never):Int;

	/**
	 * Current measure index (integer).
	 */
	public static var curMeasure(default, null):Int = 0;

	/**
	 * Current measure position (float for precision).
	 */
	public static var curMeasureFloat(default, null):Float = 0.0;

	/**
	 * Current beat index (integer).
	 */
	public static var curBeat(default, null):Int = 0;

	/**
	 * Current beat position (float for precision).
	 */
	public static var curBeatFloat(default, null):Float = 0.0;

	/**
	 * Current step index (integer).
	 */
	public static var curStep(default, null):Int = 0;

	/**
	 * Current step position (float for precision).
	 */
	public static var curStepFloat(default, null):Float = 0.0;

	/**
	 * Duration of a measure in milliseconds.
	 */
	public static var measureLengthMs(get, never):Float;

	/**
	 * Duration of a beat in milliseconds.
	 */
	public static var beatLengthMs(get, never):Float;

	/**
	 * Duration of a step in milliseconds.
	 */
	public static var stepLengthMs(get, never):Float;

	/**
	 * Offset in milliseconds to sync audio with visuals.
	 */
	public static var offset:Float = 0.0;

	/**
	 * List of BPM changes configured for the current song.
	 */
	static var bpmChanges(default, null):Array<BPMData> = [];

	/**
	 * Current BPM change being applied.
	 */
	static var curBPMChange(get, never):Null<BPMData>;

	/**
	 * Index of the current BPM change in bpmChanges array.
	 */
	static var curBPMChangeIndex(default, null):Int = 0;

	/**
	 * Internal timer for standalone mode.
	 */
	static var conductorElapsed:Float = 0.0;

	/**
	 * Whether the Conductor is currently updating.
	 */
	static var _paused:Bool = true;

	/**
	 * Whether the Conductor runs independently from FlxG.sound.music.
	 */
	static var _standalone:Bool = false;

	/**
	 * Initializes the Conductor for global use.
	 * Called once at game startup.
	 */
	@:allow(impostor.InitState)
	static function init():Void
	{
		FlxG.signals.preUpdate.add(update);
	}

	/**
	 * Starts or restarts the Conductor with specified settings.
	 * @param bpm Initial BPM (default: 100)
	 * @param standalone Whether to run independently from music (default: false)
	 * @param beatsPerMeasure Beats per measure (default: 4)
	 * @param stepsPerBeat Steps per beat (default: 4)
	 */
	public static function start(bpm:Float = 100.0, standalone:Bool = false, beatsPerMeasure:Int = 4, stepsPerBeat:Int = 4):Void
	{
		conductorElapsed = songPosition = curMeasureFloat = curBeatFloat = curStepFloat = curBPMChangeIndex = 0.0;
		curMeasure = curBeat = curStep = -1;
		resume();

		bpmChanges = [{
			time: 0.0,
			bpm: bpm,
			beatsPerMeasure: beatsPerMeasure,
			stepsPerBeat: stepsPerBeat
		}];

		_standalone = standalone;
	}

	/**
	 * Resets the Conductor to initial state.
	 */
	public static function reset():Void
	{
		conductorElapsed = songPosition = curMeasureFloat = curBeatFloat = curStepFloat = curBPMChangeIndex = 0.0;
		curMeasure = curBeat = curStep = -1;
		pause();

		bpmChanges = [];

		onMeasureHit.removeAll();
		onBeatHit.removeAll();
		onStepHit.removeAll();
	}

	/**
	 * Pauses the Conductor.
	 */
	public static function pause():Void
	{
		_paused = true;
	}

	/**
	 * Resumes the Conductor.
	 */
	public static function resume():Void
	{
		_paused = false;
	}

	/**
	 * Internal update loop called every frame.
	 */
	static function update():Void
	{
		if (_paused) return;

		if (_standalone) conductorElapsed += FlxG.elapsed;

		var curSongTime:Float = _standalone ? conductorElapsed : (FlxG.sound.music != null ? FlxG.sound.music.time : 0.0);
		var curSongLength:Float = _standalone ? Math.POSITIVE_INFINITY : (FlxG.sound.music != null ? FlxG.sound.music.length : 0.0);

		var oldMeasure:Int = curMeasure;
		var oldBeat:Int = curBeat;
		var oldStep:Int = curStep;

		if (FlxG.sound.music != null && FlxG.sound.music.playing)
		{
			songPosition = FlxMath.bound(Math.min(offset, 0.0), curSongTime, curSongLength);
			conductorElapsed += FlxG.elapsed * 1000.0 * FlxG.sound.music.pitch;
		}

		curStepFloat = FlxMath.roundDecimal((curSongTime / stepLengthMs), 4);
		curBeatFloat = curStepFloat / stepsPerBeat;
		curMeasureFloat = curBeatFloat / beatsPerMeasure;

		curStep = Math.floor(curStepFloat);
		curBeat = Math.floor(curBeatFloat);
		curMeasure = Math.floor(curMeasureFloat);

		if (curStep != oldStep) onStepHit.dispatch(curStep);
		if (curBeat != oldBeat) onBeatHit.dispatch(curBeat);
		if (curMeasure != oldMeasure) onMeasureHit.dispatch(curMeasure);

		for (i in 0...bpmChanges.length)
		{
			if (songPosition >= bpmChanges[i].time) curBPMChangeIndex = i;
			if (songPosition < bpmChanges[i].time) break;
		}
	}

	static function get_curBPMChange():BPMData
	{
		return bpmChanges[curBPMChangeIndex];
	}

	static function get_curBPM():Float
	{
		return curBPMChange.bpm;
	}

	static function get_beatsPerMeasure():Int
	{
		return curBPMChange.beatsPerMeasure;
	}

	static function get_stepsPerBeat():Int
	{
		return curBPMChange.stepsPerBeat;
	}

	static function get_measureLengthMs():Float
	{
		return beatLengthMs * beatsPerMeasure;
	}

	static function get_beatLengthMs():Float
	{
		return (60.0 / curBPM) * 1000.0;
	}

	static function get_stepLengthMs():Float
	{
		return beatLengthMs / stepsPerBeat;
	}
}

/**
 * Data structure for BPM change events.
 */
typedef BPMData =
{
	var time:Float;
	var bpm:Float;
	var beatsPerMeasure:Int;
	var stepsPerBeat:Int;
}